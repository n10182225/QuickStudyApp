...
namespace QuickStudyApp.ViewModels
{
    public class CalendarViewModel : CalendarModelControls
    {
        ...
        static AssignmentDueData databaseDue;
        ...
        public static AssignmentDueData DatabaseDue
        {
            get
            {
                if (databaseDue == null)
                {
                    databaseDue = new AssignmentDueData(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Notes.db3"));
                }
                return databaseDue;
            }
        }
